...
Link: https://arxiv.org/pdf/1804.07573.pdf
CREDITS: https://github.com/zye1996/Mobilefacenet-TF2-coral_tpu
'''
...
def conv_block(x_in, d_in, d_out, kernel_size, stride, padding, depthwise=False, linear=False):
    ...
    if depthwise:
        ...
        x = keras.layers.Conv2D(filters=d_out, kernel_size=kernel_size, strides=stride, padding='valid',
                                use_bias=False, kernel_regularizer=l2(weight_decay))(x)
...
def build_mobilefacenet(x_in):
    # cls_num = 10572
    x = inputs = tf.keras.layers.Input(shape=x_in)
    x = mobilefacenet(x)
